...
class Lista{
	...
	public void selecao(){
		for(int i=0; i < n-1 ; i++){
			int menor=i;
			for(int j=i+1;j<n;j++){
				if( array[j].getNome().compareTo(array[menor].getNome()) < 0 ) menor=j;
			}//fim for
			swap(menor,i);
		}//fim for
...
public class Tp1p2q7{
	public static void main(String[] args)throws Exception{
		...
		lista.selecao();
		String entradaPadrao2=MyIO.readLine();	
		String op="";
		while(!entradaPadrao2.contains("FIM")){
			if (lista.pesquisaBinaria(op = ISO88591toUTF8(entradaPadrao2))) MyIO.println("SIM");
			else MyIO.println("NÃO");
			entradaPadrao2=MyIO.readLine();
		}//fim while
	}//fim main

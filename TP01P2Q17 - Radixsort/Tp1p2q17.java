...
class Lista{
	...
	public void selecaoRecursiva(){
		selecaoRecursiva(0);
	}//fim public selecaoRecursiva()

	private void selecaoRecursiva(int i){
		int menor=0;
		if(i<n-1){
			loopInterno(i+1,i);
			selecaoRecursiva(i+1);
		}//fim if
	}//fim selecaoRecursiva
	private void loopInterno(int j , int menor){
		int x=0;
		if(j<n){
			if(array[j].getId() < array[menor].getId()){
				loopInterno(j+1,j);
				swap(menor,j);
			}//fim if
			loopInterno(j+1,menor);
	...
	public void insercao(){
		...
		for(int i=1;i<n;i++){
			temp=array[i];
			int j=i-1;
			while(( j >= 0 ) && ( array[j].getPaginaTam() > temp.getPaginaTam( ) ) ){
				array[j+1]=array[j];
				j--;
			}//fim while
			array[j+1]=temp;
	...
	public void quicksort(){
		quicksort(0,n-1);
	}//fim public quicksort
	private void quicksort(int esq, int dir){
		int i=esq, j=dir; 
		Presidente pivo = array[(dir+esq)/2];
		while(i<=j){
			while(array[i].getInicioMandato().getYear() < pivo.getInicioMandato().getYear()   || ( (array[i].getInicioMandato().getYear() == pivo.getInicioMandato().getYear()) && (array[i].getInicioMandato().getMonthValue() < pivo.getInicioMandato().getMonthValue()) ) || ( (array[i].getInicioMandato().getYear() == pivo.getInicioMandato().getYear()) && (array[i].getInicioMandato().getMonthValue() == pivo.getInicioMandato().getMonthValue()) && (array[i].getInicioMandato().getDayOfMonth() < pivo.getInicioMandato().getDayOfMonth())))  i++;
			while(array[j].getInicioMandato().getYear() > pivo.getInicioMandato().getYear()   || ( (array[j].getInicioMandato().getYear() == pivo.getInicioMandato().getYear()) && (array[j].getInicioMandato().getMonthValue() > pivo.getInicioMandato().getMonthValue()) ) || ( (array[j].getInicioMandato().getYear() == pivo.getInicioMandato().getYear()) && (array[j].getInicioMandato().getMonthValue() == pivo.getInicioMandato().getMonthValue()) && (array[j].getInicioMandato().getDayOfMonth() > pivo.getInicioMandato().getDayOfMonth())))  j--;
			if(i<=j){ 
				swap(i,j);
				i++;
				j--;
	...
	public void countingSort(int expoente){
		//Array para contar o numero de ocorrencias de cada elemento
		int [] contagem = new int[getMaior()+1];
		Presidente [] saida = new Presidente[n];
		//somar a quantidade de elementos iguais a array[i] no array de contagem
		for(int i=0; i<n;i++){
			contagem[(array[i].getId()/expoente) % 10]++;
		}//fim for
		//fazer com que o array de contagem some a quantidade com o array na pos anterior
		for(int i=1;i<contagem.length;i++){
			contagem[i] += contagem[i-1];
		}//fim for
		//ordenando o array de saida
		for(int i= n-1; i>=0 ; i--){
			saida[contagem[ (array[i].getId()/expoente) %10] -1 ] = array[i];
			contagem[(array[i].getId()/expoente)%10]--;
		}//fim for
		for(int i =0;i<n;i++){
			array[i]=saida[i];
	...
	public void bolha(){
		for(int i=n-1;i>0;i--){
			for(int j = 0;j<i;j++){
				if(array[j].getLocalNascimento().compareTo(array[j+1].getLocalNascimento())>0)swap(j,j+1);
				else if(array[j].getLocalNascimento().equals(array[j+1].getLocalNascimento())){
					if(array[j].getNome().compareTo(array[j+1].getNome())>0) swap(j,j+1);
				}//fim elseif
			}//fim for
		}//fim for
	}//fim bolha()
	public void mergesort(){
		Presidente [] auxiliar = new Presidente[n];
		mergesort(auxiliar,0,n-1);	
	}//fim mergesort()
	public void mergesort(Presidente[] auxiliar, int inicio, int fim){
		if(inicio<fim){
			int meio= ((inicio+fim)/2);
			mergesort(auxiliar,inicio,meio);
			mergesort(auxiliar,meio+1,fim);
			intercalar(auxiliar,inicio,meio,fim);
		}//fim if
	}//fim mergesort(3 parametors)
	public void intercalar(Presidente[] auxiliar, int inicio, int meio, int fim){
		 //inicializando o auxiliar com os valores do array
		...
		for(int k = inicio; k <= fim; k++){
			if(i > meio) array[k] = auxiliar[j++];
			else if(j> fim) array[k] = auxiliar[i++];
		             else if (auxiliar[i].getFimMandato().getYear() < auxiliar[j].getFimMandato().getYear()   || ( (auxiliar[i].getFimMandato().getYear() == auxiliar[j].getFimMandato().getYear()) && (auxiliar[i].getFimMandato().getMonthValue() < auxiliar[j].getFimMandato().getMonthValue()) ) || ( (auxiliar[i].getFimMandato().getYear() == auxiliar[j].getFimMandato().getYear()) && (auxiliar[i].getFimMandato().getMonthValue() == auxiliar[j].getFimMandato().getMonthValue()) && (auxiliar[i].getFimMandato().getDayOfMonth() < auxiliar[j].getFimMandato().getDayOfMonth()))) array[k] = auxiliar[i++];
			     else array[k]= auxiliar[j++];
		}//fim for 

	}//fim intercalar
	public int getMaior(){
		int maior=array[0].getId();
		for(int i=1;i<n;i++){
			if (array[i].getId() > maior ) maior=array[i].getId();
		}//fim for	
		return maior;
	}//fim getMaior	

...
class Lista{
	...
	public void countingSort(){
		...
		for(int i= 0; i<n; i++){
			saida[contagem[array[i].getIdade()]-1] = array[i];
			contagem[array[i].getIdade()]--;
		}//fim for
		for(int i =0;i<n;i++){
			array[i]=saida[i];
		...
		for(int i=0;i<n;i++){
			switch(array[i].getIdade()){
				case 89:if(array[i+1].getNome().compareTo(array[i].getNome())<0) swap(i,i+1);
					i++;
				break;
				case 120:if(array[i+1].getNome().compareTo(array[i].getNome())<0)swap(i,i+1);
 					 i++;
				break;
				case 151:if(array[i+1].getNome().compareTo(array[i].getNome())<0)swap(i,i+1);
                                         i++;
                                break;
				case 178:if(array[i+1].getNome().compareTo(array[i].getNome())<0)swap(i,i+1);
                                         i++;
                                break;
			}//fim switch
		}//fim for
	...
	public int getMaior(){
		int maior=array[0].getIdade();
		for(int i=1;i<n;i++){
			if (array[i].getIdade() > maior ) maior=array[i].getIdade();
		}//fim for	
		return maior;
	}//fim getMaior	
